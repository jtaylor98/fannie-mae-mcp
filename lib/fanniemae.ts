...
export interface ApiCatalogEntry {
  ...
  /**
   * Hand-recorded result of probing the live API. "unreachable" paints the
   * catalog card amber and shows a banner in the detail widget.
   *
   * Not auto-probed: checking every API on each catalog render costs a round
   * of upstream calls, and several probes return megabytes. Re-verify by hand
   * and update the date in statusNote.
   */
  status?: "ok" | "unreachable";
  /** Explanation shown when status is "unreachable". */
  statusNote?: string;
}
...
export const API_CATALOG: ApiCatalogEntry[] = [
  ...
  { name: "Manufactured Housing Loans API", tag: "Single Family", description: "Manufactured Housing Loan data including search by specification, acquisition data, performance data.", implemented: true, status: "unreachable", statusNote: "Fannie Mae gateway returns 404 ApplicationNotFound: \"Unable to identify proxy for host: secure and url: /v1/manufactured-housing-loans/...\". No proxy is deployed at that base path. Paths match the published swagger, so this is upstream, not a connector bug. Verified 2026-07-23.", operations: [
